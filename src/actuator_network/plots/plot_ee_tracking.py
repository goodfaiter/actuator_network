...
offset = 280
length = 1500
pure_df = read_mcap_to_dataframe("/workspace/src/actuator_network/plots/data/mlp_30_pure_rosbag2_2026_02_28-09_41_03_0_processed_0.mcap", topics=["/measured_position_rad_data"])
pure_df = pure_df.groupby(pure_df.index).first()
pure_df = pure_df.iloc[start : start + length]

trans_df = read_mcap_to_dataframe("/workspace/src/actuator_network/plots/data/mlp_30_model_steps_rosbag2_2026_02_28-09_30_03_0_processed_0.mcap", topics=["/measured_position_rad_data", "/desired_ee_angle_rad_data"])
...
# ax1.plot(time, trans_df["desired_ee_angle_rad_data_data"], label="Desired Finger Angle", color="green", linewidth=LINEWIDTH)
