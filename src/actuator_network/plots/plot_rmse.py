...
def plot_minimap(des):
    ...
    # Save the plot
    plt.savefig(f"/workspace/src/actuator_network/plots/figures/rmse_minimap.png", dpi=500, bbox_inches='tight')
...
def plot(dfs: list, labels: list, file_prefix: str = "", title: Optional[str] = None, legend: bool = True):
    ...
    for df, label in zip(dfs, labels):
        plt.plot(time_axis, df["load_newton_data_predicted_data"], label=f'{label}', marker='', linestyle='-', alpha=0.8, markersize=4)

    plt.plot(time_axis, dfs[0]["load_newton_data_data"], label=f'Measured', marker='', linestyle='-', markersize=4)
    ...
    # Save the plot
    plt.savefig(f"/workspace/src/actuator_network/plots/figures/{file_prefix}_actual_vs_predicted.png", dpi=500, bbox_inches='tight')

def plot_bars(nums: Dict[str, List[float]], stds: Dict[str, List[float]], labels: List[str], file_prefix: str = "", title: Optional[str] = None):
    """
...
for mcap_file_path in mcap_file_paths:
    data_df = read_mcap_to_dataframe(mcap_file_path, topics=["/desired_position_rad_data", "/load_newton_data", "/load_newton_data_predicted"])
