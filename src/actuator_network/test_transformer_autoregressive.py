"""Run closed-loop autoregressive Transformer inference on test MCAPs."""
...
DEFAULT_MODEL_PATH = "/workspace/data/output_data/best_transformer_autoregressive_latest.pt"
...
def run_transformer_autoregressive_inference(
    ...
    ar_idx = input_cols.index(ar_col)
    ...
    for mcap_file_path in mcap_file_paths:
        ...
        predictions = torch.zeros((num_samples, len(output_cols)), device=device)
        ...
        for t in range(window_span - 1, num_samples):
            ...
            window = features[indices].clone()
            # Feed back previous predictions for the autoregressive force channel.
            ar_indices = (indices - 1).clamp_min(0)
            window[:, ar_idx] = predictions[ar_indices, 0]
        ...
        for i, col in enumerate(output_cols):
            data_df_extrapolated[col + "_predicted"] = predictions[:, i].cpu().numpy()

        output_path = mcap_file_path.replace(".mcap", "_transformer_autoregressive_predicted.mcap")
