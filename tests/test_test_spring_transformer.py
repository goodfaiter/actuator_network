...
def test_run_spring_transformer_inference_frozen_latent(tmp_path):
    """Inference on a pruned checkpoint with an embedded latent pins the spring
    channel to one value."""
    assert os.path.isfile(TEST_MCAP), f"Test MCAP not found: {TEST_MCAP}"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Build the pruned deployment checkpoint with an embedded latent.
        loaded_full = torch.jit.load(full_model_path)
